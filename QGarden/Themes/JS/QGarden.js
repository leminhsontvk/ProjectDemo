...
window.onload = function ()
{
    ...
    $('#UserInfoChange').click(async function (Event)
    {
        ...
        if ($('#input-avatar')['0'].files.length === 0 && LoginName === "" && UserName === "" && UserAddress === "" && UserPhone === "" && UserBirthday === "" && UserMail === "" && UserCurrentPass === "")
        {
        ...
        else
        {
            ...
            if (LoginName !== "")
            {
                if (UserLoginRegex.test(LoginName) !== true)
                {
                    ErrorMessage += "Tên Đăng nhập không hợp lệ. Tên đăng nhập chỉ chứa các ký tự chữ và số. <br>";
                }
                                ...
                                (
                                    {
                                        type: 'success',
                                        title: 'Thành Công',
                                        text: 'Thay đổi thông tin tài khoản thành công. Vui lòng đăng xuất và đăng nhập trở lại để thay đổi có hiệu lực.',
                                        showConfirmButton: true,
                                        showCancelButton: false,
                                    }
                                );
    ...
    $('#DoRegister').click(async function (Event)
    {
        ...
        else
        {
            ...
            if (UserLogin !== "")
            {
                if (UserLoginRegex.test(UserLogin) !== true)
                {
                    ErrorMessage += "Tên Đăng nhập không hợp lệ. Tên đăng nhập chỉ chứa các ký tự chữ và số. <br>";
                }
            ...
            if (UserMail !== "")
            {
                if (MailRegex.test(UserMail) === false) ErrorMessage += "Địa chỉ mail không hợp lệ. <br>";
            ...
            if (UserName !== "") Register.append('UserName', UserName); else ErrorMessage += "Vui lòng nhập họ và tên. <br>";
    ...
    $('#CheckOut').click(async function (Event)
    {
        ...
        if (UserPhone === "") ErrorMessage += "Vui lòng nhập số điện thoại.<br>";
        if (UserAddress === "") ErrorMessage += "Vui lòng nhập địa chỉ.<br>";
        if (UserName === "") ErrorMessage += "Vui lòng nhập họ và tên.<br>";

        if (UserPhone !=="" && PhoneRegex.test(UserPhone) !== true)
        {
            ErrorMessage += "Số điện thoại không hợp lệ.";
        }

        if (UserMail !=="" && MailRegex.test(UserMail) !== true)
        {
        ...
        CheckoutData.append('UserName', UserName);
            ...
            (
                {
                    type: 'warning',
                    title: 'Oops',
                    text: 'Vui lòng nhập LoginID hoặc Email đã đăng ký.',
                    showConfirmButton: true,
                }
            ); return;
        }
